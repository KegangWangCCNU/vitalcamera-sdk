/**
 * @file inference.worker.js
 * @description rPPG inference Web Worker for the VitalCamera SDK.
 *
 * Runs the rPPG neural network model (EfficientPhys / ContrastPhys variant)
 * inside a dedicated Web Worker thread using LiteRT (TFLite WASM backend).
 *
 * The worker manages a complex recurrent state tensor graph: 48 input tensors
 * are fed each frame, and STATE_MAP defines how output tensors cycle back as
 * inputs for the next step. An optional projection model extracts intermediate
 * feature maps for visualization.
 *
 * Message protocol (type / payload):
 *   -> init    { modelBuffer, stateJson, projBuffer }  Load models + restore state
 *   <- initDone                                         Models ready
 *   -> run     { imgData, dtVal, timestamp }            Run one frame
 *   <- result  { step, value, time, projOutput, timestamp }
 *   -> export_state                                     Serialize current state
 *   <- state_exported { ...stateSnapshot }
 *   -> setMode { isLowPower }                           Toggle low-power mode
...
/** Keys used to label the projection model's output tensors. */
const PROJ_OUTPUT_KEYS = ['fm1', 'fm2', 'fm3', 'fm4', 'proj', 'ssm1', 'ssm2', 'ssm3', 'ssm4'];

/**
 * Maps how output tensors from the main model are recycled as input tensors
 * for the next inference step.  { inIdx: input slot, outIdx: output slot }.
 */
const STATE_MAP = [
...
/* ── Exported for testing (Node.js only, no-op in browser worker context) ── */
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { STATE_MAP, INPUT_COUNT, IMG_IDX, DT_IDX, PROJ_INPUT_SOURCE_INDICES };
}
...
/**
 * Initialize the rPPG model and projection model.
 * @param {Object} params
 * @param {ArrayBuffer} params.modelBuffer  - Main rPPG model (.tflite)
 ...
 * @param {Object} params
 * @param {Float32Array} params.imgData    - Preprocessed face patch (36x36x3)
 * @param {number}       params.dtVal      - Time delta since last frame (seconds)
 * @param {number}       params.timestamp  - Original frame timestamp (ms)
 */
async function handleRun({ imgData, dtVal, timestamp }) {
    ...
    // Recycle output tensors back into input slots per STATE_MAP
    ...
    // Run projection model (skip in low-power mode)
    let formattedOutput = {};
    if (!lowPowerMode) {
...
/**
 * Export the current recurrent state as a JSON-serializable snapshot.
 * Used for persistence / session restore.
 */
